...
export class MantoInformeSemanalJspdfService {
  ...
  generaPDFTransaccional(month, year, chartData, tblData, barraData){
      let doc = new jsPDF({
        orientation: "l",
        unit: "mm",
        format: [150, 240]
      ...
      //tabla
      doc.addImage(tblData, 'PNG', 6, 24, 160, 120);
     
      let filename = "informe_mantencion_BO_TRX_";       
      filename += month + year + ".pdf";

      doc.save(filename);
  }

  generaPDFComercial(month, year, chartData, tblData, barraData, tablaDetalles){
    let doc = new jsPDF({
      orientation: "l",
      unit: "mm",
      format: [150, 240]
    ...
    head = [[
      'Servicio',
      'Consumo de horas',
      '% sobre las horas presupuestadas'
    ]];
